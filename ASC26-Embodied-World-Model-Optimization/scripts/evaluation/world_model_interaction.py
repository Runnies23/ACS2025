...
def run_inference(args: argparse.Namespace, gpu_num: int, gpu_no: int) -> None:
    ...
    if hasattr(args, 'dataset') and args.dataset:
        if 'params' not in config.data:
            config.data['params'] = {}
        config.data['params']['target_dataset'] = args.dataset
        print(f">>> Setting target_dataset to: {args.dataset}")

    # Pass prompt_dir
    if hasattr(args, 'prompt_dir') and args.prompt_dir:
        if 'params' not in config.data:
            config.data['params'] = {}
        config.data['params']['prompt_dir'] = args.prompt_dir
        print(f">>> Setting prompt_dir to: {args.prompt_dir}")
