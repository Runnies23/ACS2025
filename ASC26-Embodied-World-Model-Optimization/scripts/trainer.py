...
if __name__ == "__main__":
    ...
    # Safe rank/env defaults (works for single-GPU interactive runs and DDP launches)
    ...
    # Load configs (YAML + CLI overrides)
    ...
    # Freeze everything, unfreeze cross-attention only
    ...
    # Register_schedule again to make ZTSNR work (if used)
    ...
    # Propagate any non-default trainer args passed on CLI
    for k in get_nondefault_trainer_args(args):
        trainer_config[k] = getattr(args, k)

    # FORCE single-GPU mode (important on this cluster)
    # This ensures Lightning does NOT try to use sharded strategies / FairScale.
    trainer_config["accelerator"] = "gpu"
    trainer_config["devices"] = 1
    trainer_config["num_nodes"] = 1

    # Remove any configured strategy to avoid 'ddp_sharded' / FairScale being used.
    # Some configs set lightning.trainer.strategy; remove it to force single-device behaviour.
    if "strategy" in trainer_config:
        trainer_config.pop("strategy", None)
    if "strategy" in lightning_config:
        lightning_config.pop("strategy", None)

    logger.info(f"Running on {num_rank}={trainer_config['num_nodes']}x{trainer_config['devices']} GPUs (forced single-GPU mode)")

    # Setup learning rate (num_rank will be 1 here)
    ...
    # If user provided --total_gpus=... on CLI, read it (not necessary for single GPU)
    for item in unknown:
    ...
    # Ensure accelerator is set (redundant but safe)
    trainer_config.setdefault("accelerator", "gpu")

    # Create trainer kwargs (do NOT include any sharded strategy)
    trainer_kwargs = dict()
    ...
    # Note: we DO NOT set trainer_kwargs["strategy"] here for single-GPU
    trainer_kwargs["precision"] = lightning_config.get("precision", 32)
    trainer_kwargs["sync_batchnorm"] = False

    # Build Trainer args and instantiate Trainer
    trainer_args = argparse.Namespace(**trainer_config)
    logger.info(f"Trainer args: {trainer_config}")
    ...
    # Show model sizes & run
    total_params = get_num_parameters(model)
    logger.info("***** Running the Loop *****")
    if args.train:
        try:
            # Single-GPU: just call fit
            logger.info("<Training in single-GPU mode>")
