...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def __init__(self,
                 ...
                 target_dataset=None,
                 prompt_dir=None):  # ADDED: prompt_dir parameter
        super().__init__()
        ...
        self.prompt_dir = prompt_dir  # ADDED: Store prompt_dir
    ...
    def setup(self, stage=None):
        ...
        if self.target_dataset:
            print(f">>> Loading only target dataset: {self.target_dataset}")
            datasets_to_load = [self.target_dataset]
        else:
            print(f">>> Loading all datasets from config")
            datasets_to_load = list(self.dataset_weights.keys())
        
        # MODIFIED: Use prompt_dir directly for all paths
        if self.prompt_dir:
            # Convert to absolute path if it's relative
            from pathlib import Path
            prompt_dir_abs = Path(self.prompt_dir).resolve()
            print(f">>> Using prompt_dir: {self.prompt_dir}")
            print(f">>> Absolute path: {prompt_dir_abs}")
            data_dir_override = str(prompt_dir_abs)
        else:
            data_dir_override = None
        
        if 'train' in self.dataset_configs:
            self.train_datasets = dict()
            for dataname in datasets_to_load:
                if dataname not in self.dataset_weights:
                    print(f"Warning: {dataname} not in dataset_weights, skipping")
                    continue
                
                # MODIFIED: Use prompt_dir directly if available
                if data_dir_override:
                    data_dir = data_dir_override
                else:
                    data_dir = self.dataset_configs['train']['params']['data_dir']
                
                # CSV file is inside prompt_dir (or data_dir)
                csv_file = f'{dataname}.csv'
                meta_path = os.path.join(data_dir, csv_file)
                
                # Transitions are in prompt_dir/transitions (or data_dir/transitions)
                transition_dir = os.path.join(data_dir, 'transitions')
                ...
                self.dataset_configs['train']['params']['meta_path'] = meta_path
                self.dataset_configs['train']['params']['transition_dir'] = transition_dir
                self.dataset_configs['train']['params']['data_dir'] = data_dir
                self.dataset_configs['train']['params']['dataset_name'] = dataname
        ...
        if 'validation' in self.dataset_configs:
            ...
            for dataname in datasets_to_load:
                ...
                if data_dir_override:
                    ...
                    data_dir = self.dataset_configs['validation']['params']['data_dir']
                
                csv_file = f'{dataname}.csv'
                meta_path = os.path.join(data_dir, csv_file)
                transition_dir = os.path.join(data_dir, 'transitions')
                
                self.dataset_configs['validation']['params']['meta_path'] = meta_path
                self.dataset_configs['validation']['params']['transition_dir'] = transition_dir
                self.dataset_configs['validation']['params']['data_dir'] = data_dir
                self.dataset_configs['validation']['params']['dataset_name'] = dataname
        ...
        if 'test' in self.dataset_configs:
            ...
            for dataname in datasets_to_load:
                ...
                self.dataset_configs['test']['params']['transition_dir'] = transition_dir
                self.dataset_configs['test']['params']['data_dir'] = data_dir
                self.dataset_configs['test']['params']['dataset_name'] = dataname
