...
from unifolm_wma_run.utils.diffusion import timestep_embedding
from unifolm_wma_run.utils.common import checkpoint
from unifolm_wma_run.utils.basics import (zero_module, conv_nd, linear,
                                      avg_pool_nd, normalization)
from unifolm_wma_run.modules.attention import SpatialTransformer, TemporalTransformer
...
class WMAModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.action_unet = instantiate_from_config(unet_head_config) #ConditionalUnet1D
        ...
        logging.info("[INFO] this is the model architecture")
        # print(self.input_blocks)
        logging.info("[INFO] input_blocks")
        logging.info(self.input_blocks)

        # print(self.output_blocks)
        logging.info("[INFO] output_blocks")
        logging.info(self.output_blocks)

        # print(self.action_unet)
        logging.info("[INFO] action_unet")
        logging.info(self.action_unet)
        logging.info("[INFO] Done load WMAmodel")
    ...
    def forward(self,
        ...
        """
        logging.info("[INFO] apply model on WMAModel")
        b, _, t, _, _ = x.shape
        with timer("timestep_embedding"):
            t_emb = timestep_embedding(timesteps,
                                    self.model_channels,
                                    repeat_only=False).type(x.dtype)
        with timer("time_embed"):
            emb = self.time_embed(t_emb) ##TODO
        ...
        with timer("repeat_interleave"):
            emb = emb.repeat_interleave(repeats=t, dim=0)

        with timer("rearrange_1"):
            x = rearrange(x, 'b c t h w -> (b t) c h w')

        # Combine emb
        if self.fs_condition:
            with timer("fs_condition"):
                if fs is None:
        ...
        hs = []
        hs_a = []
        logging.info(f"[INFO] Run on Iteration total{len(self.input_blocks)}")
        for id, module in enumerate(self.input_blocks):
            logging.info(f"[INFO] ========= {id}/{len(self.input_blocks)} =========")
            
            with timer("module"):# mins 0.100882s - max 0.232557s
                h = module(h, emb, context=context, batch_size=b) ##TODO 
            if id == 0 and self.addition_attention:
                with timer("init_attn"): #0.165213s
                    h = self.init_attn(h, emb, context=context, batch_size=b)
            # plug-in adapter features
            if ((id + 1) % 3 == 0) and features_adapter is not None:
                with timer("features_adapter"):
                    h = h + features_adapter[adapter_idx]
                    adapter_idx += 1
            if id != 0:
                if isinstance(module[0], Downsample): 
                    with timer("hs_a.append"): #0.000141s
                        hs_a.append(
                            rearrange(hs[-1], '(b t) c h w -> b t c h w', t=t))
            hs.append(h)

        with timer("rearrange append"): #0.000022s
        ...
        with timer("middle_block"):
            h = self.middle_block(h, emb, context=context, batch_size=b)

        with timer("rearrange append2"):
            hs_a.append(rearrange(h, '(b t) c h w -> b t c h w', t=t))

        hs_out = []
        logging.info(f"[INFO] ========= output_blocks total {len(self.output_blocks)} =========")
        for idx, module in enumerate(self.output_blocks):
        # for module in self.output_blocks:
            logging.info(f"[INFO] ========= output_blocks {idx} / {len(self.output_blocks)} =========")
            with timer("torch.cat"): #0.000058s
                h = torch.cat([h, hs.pop()], dim=1)
            with timer("module"): #min 0.054967s - max 0.254397s ##TODO
                h = module(h, emb, context=context, batch_size=b)
            if isinstance(module[-1], Upsample):
                with timer("rearrange append3"):
                    hs_a.append(
        ...
        h = h.type(x.dtype)
        with timer("append rearrange"):
            hs_a.append(rearrange(hs_out[-1], '(b t) c h w -> b t c h w', t=t))

        with timer("self.out (Sequantial)"): #0.000265s
            y = self.out(h)
        with timer("rearrange_2"): #0.000025s
            y = rearrange(y, '(b t) c h w -> b c t h w', b=b)

        if not self.base_model_gen_only:
            with timer("base_model_gen_only => get shape"):
                ba, _, _ = x_action.shape
            with timer("base_model_gen_only => action_unet"): #0.239149s ##TODO
            ...
            if b > 1:
                ...
                """
                with timer("base_model_gen_only => Predict state b > 1"):
                    s_y = self.state_unet(x_state, timesteps[:ba], hs_a,
                                        context_action[:2], **kwargs)
            else:
                with timer("base_model_gen_only => Predict state"):
                    s_y = self.state_unet(x_state, timesteps, hs_a,
                                        context_action[:2], **kwargs)
        else:
            with timer("torch.zeros_like"):
