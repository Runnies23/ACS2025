...
print("Force INTO XFORMERS IS AVAILBLE")
XFORMERS_IS_AVAILBLE = True 

from unifolm_wma_run.utils.common import (
    checkpoint,
    exists,
    default,
)
from unifolm_wma_run.utils.basics import zero_module
...
def sdpa_xformer_like(q, k, v, attn_bias=None, causal=False, scale=None):
    # match xformers scaling
    if scale is None:
        scale = q.size(-1) ** -0.5

    q = q * scale

    # convert structured masks → additive mask
    if isinstance(attn_bias, torch.Tensor):
        mask = attn_bias
    elif attn_bias is None:
        mask = None
    else:
        raise TypeError("Unsupported attn_bias type")
...
def naive_attention(query, key, value, attn_bias=None, p=0.0, training=True):
    """
    query, key, value: (B, N, H, D)
    attn_bias:
        None
        or broadcastable to (B,H,N,N)
        or bool mask (True=keep, False=mask)
    ...
    # dropout
    attn = F.dropout(attn, p=p, training=training)

    # weighted sum
    attn = attn @ value  # (B,H,N,D)

    # restore layout
    return attn.transpose(1, 2).contiguous()


def safe_attention(q, k, v, attn_bias=None, dropout_p=0.0, scale=None):
    if scale is None:
        scale = q.shape[-1] ** -0.5
...
class CrossAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if self.relative_position:
            ...
            if XFORMERS_IS_AVAILBLE and temporal_length is None:
                print("[INFO Using efficient_forward]")
                self.forward = self.efficient_forward
            else: 
                print("[INFO Using normal_forward]")
    ...
    def efficient_forward(self, x, context=None, mask=None):
        ...
        if k is not None:
            ...
            # out = safe_attention(q, k, v, attn_bias=None) #this line import torch
            ...
            #                                             attn_bias=None,
            #                                             op=None)
            #use instead of xformers.ops.memory_efficient_attention
        ...
        if k_ip is not None:
            ...
            # out_ip = naive_attention(q, k_ip, v_ip, attn_bias=None) #this line 
            # out_ip = sdpa_xformer_like(
            #     q, k_ip, v_ip,
            #     attn_bias=None,
            #     causal=False
            # )
            ...
            # out_ip = safe_attention(q, k_ip, v_ip, attn_bias=None) #this line 
            # q = q.to(torch.bfloat16)
            # k_ip = k_ip.to(torch.bfloat16)
            # v_ip = v_ip.to(torch.bfloat16)
            # with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        if k_as is not None:
            ...
            # out_as = naive_attention(q, k_as, v_as, attn_bias=None) #this line 
            # out_as = sdpa_xformer_like(
            #     q, k_as, v_as,
            #     attn_bias=None,
            #     causal=False
            # )
            ...
            # out_as = safe_attention(q, k_as, v_as, attn_bias=None) #this line 
            # q = q.to(torch.bfloat16)
            # k_as = k_as.to(torch.bfloat16)
            # v_as = v_as.to(torch.bfloat16)
            # with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        if k_aa is not None:
            ...
            # out_aa = naive_attention(q, k_aa, v_aa, attn_bias=None) #this line 
            # out_aa = sdpa_xformer_like(
            #     q, k_aa, v_aa,
            #     attn_bias=attn_mask_aa,
            #     causal=False
            # )
            ...
            # out_aa = safe_attention(q, k_aa, v_aa, attn_bias=attn_mask_aa)  #this line 
            # q = q.to(torch.bfloat16)
            # k_aa = k_aa.to(torch.bfloat16)
            # v_aa = v_aa.to(torch.bfloat16)
            # with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
