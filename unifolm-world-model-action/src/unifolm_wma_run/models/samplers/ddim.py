...
from unifolm_wma_run.utils.diffusion import make_ddim_sampling_parameters, make_ddim_timesteps, rescale_noise_cfg
from unifolm_wma_run.utils.common import noise_like
from unifolm_wma_run.utils.common import extract_into_tensor
...
class DDIMSampler(object):
    ...
    def sample(
        ...
        # Check condition bs
        logging.info("[INFO] DDIMSampler.sample")

        with timer("prepare input of ddim_sampling"):
    ...
    def ddim_sampling(self,
                      ...
                      guidance_rescale=0.0,
                        ...
                        state_model: str = "last",
                      **kwargs):
        with timer("prepare input of feeding"):
        ...
        logging.info(f"[INFO] ========= Iteration {len(iterator)} in total =========")

        for i, step in enumerate(iterator):
            logging.info(f"[INFO] ========= Iteration {i}/{len(iterator)} =========")
            ...
            logging.info("[INFO] p_sample_ddim")
    ...
    def p_sample_ddim(self,
        ...
        os.makedirs(os.path.join('./compare',  datasets_name, device_name, state_model),exist_ok=True)
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            logging.info("[INFO] unconditional_conditioning")
            # with timer("unconditional_conditioning"):
            model_output, model_output_action, model_output_state = self.model.apply_model(
            x, x_action, x_state, t, c, **kwargs)  # unet denoiser

            #save model each iterator (total50)
            torch.save(model_output, os.path.join('./compare',  datasets_name, device_name, state_model, f"{iter_idx}-{iterator}.pt"))



        else:
            with timer("do_classifier_free_guidance"):
        ...
        with timer("predict_eps_from_z_and_v"):
        ...
        if score_corrector is not None:
            with timer("score_corrector"):
                assert self.model.parameterization == "eps", 'not implemented'
                e_t = score_corrector.modify_score(self.model, e_t, x, t, c,
                                                **corrector_kwargs)

        with timer("alphas_cumprod"):
            alphas = self.model.alphas_cumprod if use_original_steps else self.ddim_alphas
        with timer("alphas_cumprod_prev"):
            alphas_prev = self.model.alphas_cumprod_prev if use_original_steps else self.ddim_alphas_prev
        with timer("sqrt_one_minus_alphas_cumprod"):
            sqrt_one_minus_alphas = self.model.sqrt_one_minus_alphas_cumprod if use_original_steps else self.ddim_sqrt_one_minus_alphas
        with timer("ddim_sigmas_for_original_num_steps"):
        ...
        ##create empty tensor 
        with timer("create empty tensor "):
        ...
        if self.model.use_dynamic_rescale:
            with timer("use_dynamic_rescale"):
        ...
        if quantize_denoised:
            with timer("first_stage_model.quantize"):
                pred_x0, _, *_ = self.model.first_stage_model.quantize(pred_x0)

        with timer("tensor sqrt.quantize"):
        ...
        if noise_dropout > 0.:
            with timer("noise dropout"):
                noise = torch.nn.functional.dropout(noise, p=noise_dropout)

        with timer("math apply"):
