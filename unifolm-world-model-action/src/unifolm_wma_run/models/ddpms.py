...
from unifolm_wma_run.utils.ema import LitEma
from unifolm_wma_run.utils.distributions import DiagonalGaussianDistribution
from unifolm_wma_run.utils.diffusion import make_beta_schedule, rescale_zero_terminal_snr
from unifolm_wma_run.utils.basics import disabled_train
from unifolm_wma_run.utils.common import (extract_into_tensor, noise_like, exists,
                                      default)

from unifolm_wma_run.models.samplers.ddim import DDIMSampler
from unifolm_wma_run.models.diffusion_head.common.lr_scheduler import get_scheduler, SelectiveLRScheduler
from unifolm_wma_run.models.diffusion_head.ema_model import EMAModel
from unifolm_wma_run.models.diffusion_head.positional_embedding import SinusoidalPosEmb
from unifolm_wma_run.modules.encoders.condition import MLPProjector
from unifolm_wma_run.data.normolize import Normalize, Unnormalize
import time
...
class DDPM(pl.LightningModule):
    ...
    def p_mean_variance(self, x: Tensor, t: Tensor,
        ...
        """
        logging.info(f"[INFO] on p_mean_variance")
        with timer("self.model"):
            model_out = self.model(x, t)

        if self.parameterization == "eps":
            with timer("predict_start_from_noise"):
                x_recon = self.predict_start_from_noise(x, t=t, noise=model_out)

        elif self.parameterization == "x0":
            with timer("use the model output"):
                    x_recon = model_out

        if clip_denoised:
            with timer("clip_denoised"):
                x_recon.clamp_(-1., 1.)

        with timer("q_posterior"):
    ...
    def p_sample(self,
        ...
        Returns:
            Next sample x_{t-1}.
        """
        logging.info(f"[INFO] on p_sample")
        with timer("get shape & device"):
            b, *_, device = *x.shape, x.device

        ##have profiling on function
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, t=t, clip_denoised=clip_denoised)
        
        with timer("noise_like"):
            noise = noise_like(x.shape, device, repeat_noise)
        # No noise when t == 0
        with timer("reshape"):
            nonzero_mask = (1 - (t == 0).float()).reshape(
                b, *((1, ) * (len(x.shape) - 1)))
        with timer("matht return"):
            math_return = model_mean + nonzero_mask * (0.5 *
    ...
    def p_sample_loop(
        ...
        b = shape[0]
        with timer("rand tensor"):
            img = torch.randn(shape, device=device)
        intermediates = [img]
        logging.info(f"[INFO]run on num_timesteps on {self.num_timesteps} in total")
        for i in tqdm(reversed(range(0, self.num_timesteps)),
                    desc='Sampling t',
                    total=self.num_timesteps):
            logging.info(f"[INFO]run on num_timesteps on {i}/{self.num_timesteps} ")
            ...
            with timer("append intermediates"):
                if i % self.log_every_t == 0 or i == self.num_timesteps - 1:
                    intermediates.append(img)

        logging.info("[INFO] Done p_sample_loop")
        if return_intermediates:
            logging.info("[INFO] return intermediates")
            return img, intermediates
        
        logging.info("[INFO] not return intermediates")
        return img
    ...
    def sample(
        ...
        logging.info("[INFO] p_sample_loop")
...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy: Tensor, x_action_noisy: Tensor,
        ...
        logging.info("[INFO] Done apply_model")
