...
def timer(name):
    if debug:
        ...
        try:
            ...
            logging.info(f"{name}: {end - start:.6f}s")
    else:
        yield
...
def prepare_init_input(start_idx: int,
    ...
    with timer("normalize action - state"):
...
def get_latent_z(model, videos: Tensor) -> Tensor:
    ...
    """
    logging.info("[INFO] get_latent_z function")
    b, c, t, h, w = videos.shape
    with timer("get_latent_z => rearrange video"):
        x = rearrange(videos, 'b c t h w -> (b t) c h w')
    with timer("get_latent_z => encode_first_stage"):
        z = model.encode_first_stage(x)
    with timer("get_latent_z => rearrange z"):
...
def image_guided_synthesis_sim_mode(
        ...
        sim_mode: bool = True,
        iter_idx: int = 0,
        datasets_name : str = "sample",
        state_model: str = "last",
    ...
    os.makedirs(os.path.join('./compare',  datasets_name, device_name, state_model,"c_crossattn"),exist_ok=True)


    logging.info(f"[INFO] IN image_guided_synthesis_sim_mode function")

    ##TODO  optimize by write in the main function not called function
    with timer("init model & noise"):
    ...
    if not text_input:
        prompts = [""] * batch_size
    with timer("Embedded Prompt"):
        cond_ins_emb = model.get_learned_conditioning(prompts)

    with timer("Projector state"):
    ...
    torch.save(torch.cat(
            ...
            dim=1), os.path.join('./compare', datasets_name, device_name, state_model, "c_crossattn", f"cond_input-{iter_idx}.pt"))
    ...
    # with timer("Apply model logic"):
    logging.info("[INFO] Apply model logic")
    uc = None
    kwargs.update({"unconditional_conditioning_img_nonetext": None})
    cond_mask = None
    cond_z0 = None
    if ddim_sampler is not None:
        # logging.info("[INFO] ddim_sampler.sample")
        ...
        # state_model: str = "last",
            ...
            **kwargs)
        logging.info("[INFO] Done ddim_sampler.sample")

        # Reconstruct from latent to pixel space

        with timer("model.decode_first_stage"):
            batch_images = model.decode_first_stage(samples)
            batch_variants = batch_images

    logging.info(f"[INFO] Done image_guided_synthesis_sim_mode function")
    return batch_variants, actions, states


def run_inference(args: argparse.Namespace, gpu_num: int, gpu_no: int, multi_gpu:str="") -> None:
    """
    ...
    Args:
        ...
        multi_gpu (str): type of multi gpu 

    Returns:
        None
    """

    logging.info("[INFO] Start Inference")

    #create dir for store the data
    os.makedirs(os.path.join('./compare' , args.dataset) ,exist_ok=True)
    os.makedirs(os.path.join('./compare', args.dataset, device_name),exist_ok=True)

    # Create inference and tensorboard dirs
    with timer("init valueable"):
    ...
    with timer("load models"):
        ...
        ##TODO Inprogress
        ## * Adding Multi gpu => DataParallel 
        if multi_gpu == "DataParallel":
            
            model = torch.nn.DataParallel(model)
            model = model.cuda()

        ## * Adding Multi gpu => DistributedDataParallel 
        if multi_gpu == "DistributedDataParallel":
            ##TODO assign local_rank in each gpu 
        ...
        ## * Adding Multi gpu => Batch_parallel_inference 
        if multi_gpu == "Batch_parallel_inference":
            ##TODO Batch_parallel_inference process
            pass

        ## * Adding Multi gpu => Tensor_Parallel 
        if multi_gpu == "Tensor_Parallel":
            ##TODO Tensor_Parallel process
            pass
    ...
    with timer("load model checkpoint"):
    ...
    logging.info("[INFO] Start running Inference")
    # Start inference
    for idx in range(0, len(df)):
        sample = df.iloc[idx]

        # Got initial frame path
        with timer("get_init_frame_path"):
        ...
        logging.info("[INFO] start running on loop frame_stride")
        for fs in args.frame_stride:

            with timer("mkdir => save dir"):
            ...
            with timer("init condition input w/ populate_queues"):
            ...
            logging.info("[INFO] start running on loop n_iter")
            # for itr in tqdm(range(args.n_iter)):
            for itr in range(args.n_iter):
                logging.info(f"[INFO] start {itr + 1}/{args.n_iter} n_iter (start with idx:1)")

                # Get observation
                with timer("get decision model observation"):
                    observation = {
                        'observation.images.top':
                        torch.stack(list(
                ...
                print(f'>>> Step {itr + 1}: generating actions ...')
                logging.info(f"[INFO] Step {itr + 1}: generating actions ...")
                ...
                with timer("updateing observation by decesion model action output"):
                    for idx in range(len(pred_actions[0])):
                        observation = {'action': pred_actions[0][idx:idx + 1]}
                        observation['action'][:, ori_action_dim:] = 0.0
                        with timer("update condition input w/ populate_queues"):
                ...
                print(f'>>> Step {itr + 1}: interacting with world model ...')
                logging.info(f"[INFO] Step {itr + 1}: interacting with world model ...")
                ...
                #save tensor output 
                torch.save(pred_videos_1[:, :, :args.exe_steps], os.path.join('./compare', args.dataset, device_name, f"output-{itr+1}.pt"))


                with timer("updateing observation by world model state output"):
                ...
                with timer("save output (not nesscary)"):
...
if __name__ == '__main__':
    ...
    num_gpu = count_gpu()
    if num_gpu > 1:
        multi_gpu = "DataParallel"
        run_inference(args, gpu_num, rank, multi_gpu)
    else:
        print(f"it's find only {num_gpu} gpus can't do multi gpu")

        print(f"still run it's any ways")
        run_inference(args, gpu_num, rank, "None")


    inference_end = time.perf_counter()
    logging.info(f"Run inference total time: {inference_end - inference_start:.6f}s")